...
class GeometryTool(object):
    ...
    def vectorAdd(self, vec1, vec2):

        vec = (vec1[0]+vec2[0], vec1[1]+vec2[1], vec1[2]+vec2[2])
        return vec

    def calcPointsDirection(self, p1, p2):
    ...
    def calcPointsDistance(self, p1, p2):
    ...
    def alignManhattan(self, gps):
        ...
        for i, l in enumerate(idxList):
            ...
            center = self.getPointsAvg(tmpList)
    ...
    def calcCenterRegionAvgVal(self, data, centerPos, steps):
        
        lt, rb = self.calcCenterRegionPos(centerPos, steps, data.shape)
        region = self.getRegionData(data, lt, rb)
        avgVal = np.nanmean(region)

        return avgVal

    def calcCenterRegionPos(self, centerPos, steps, size):

        lt = (centerPos[0] - steps[0], centerPos[1] - steps[1])
        rb = (centerPos[0] + steps[0], centerPos[1] + steps[1])

        lt = self.checkBoundary(lt, size)
        rb = self.checkBoundary(rb, size)

        return lt, rb

    def checkBoundary(self, pos, size):
    ...
    def getRegionData(self, data, lt, rb):
