...
class GeoPoint(object):
    ...
    def initByScene(self):
        ...
        if self.coords == None:
            self.coords = utils.PanoTool().xyz2coords(self.xyz)

        coordsT = (self.coords[1], self.coords[0])

        colorData = self.__scene.getPanoColorData()

        colorPos = utils.PanoTool().coords2pos(coordsT, colorData.shape)
        rgb = colorData[colorPos[0]][colorPos[1]]
        self.color = (rgb[0], rgb[1], rgb[2])

        depthData = self.__scene.getPanoDepthData()
        
        depthPos = utils.PanoTool().coords2pos(coordsT, depthData.shape)
        avgVal= utils.GeometryTool().calcCenterRegionAvgVal(depthData, 
                                                            depthPos, (5, 5))
        self.depth = avgVal
        #self.depth = depthData[depthPos[0]][depthPos[1]]

        if self.xyz == None:
            self.xyz = utils.PanoTool().coords2xyz(self.coords, self.depth)
    ...
    def calcGeometryType(self):
        ...
        geoTool = utils.GeometryTool()
        #print(depthPos)
        lt, rb = geoTool.calcCenterRegionPos(depthPos, 
        ...
        regionL = geoTool.getRegionData(depthData, lt, cb)
        #print(regionL.shape)
        ct = (lt[0], depthPos[1])
        #print("ct {0}".format(ct))
        regionR = geoTool.getRegionData(depthData, ct, rb)
        #print(regionR.shape)
